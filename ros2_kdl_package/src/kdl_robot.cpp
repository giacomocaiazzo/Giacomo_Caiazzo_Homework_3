...
void KDLRobot::getInverseKinematicsVel(KDL::Twist &twist, KDL::JntArray &dq){
    int ret = ikVelSol_->CartToJnt(jntArray_,twist,dq);
...
Eigen::VectorXd KDLRobot::getEEJacDotqDot()
{
    Eigen::VectorXd vec;
    vec.resize(6);
    vec <<  s_J_dot_q_dot_ee_.vel.x(),s_J_dot_q_dot_ee_.vel.y(),s_J_dot_q_dot_ee_.vel.z(),
            s_J_dot_q_dot_ee_.rot.x(),s_J_dot_q_dot_ee_.rot.y(),s_J_dot_q_dot_ee_.rot.z(); 
    return vec;
}
